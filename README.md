...
目标 P0 Demo 包含两个架空国家、双层二维地图、人物随机开局、行动与组织、简化 AI、继承、存档和开发工具。当前发布状态必须按下表理解：

- 后台模拟与状态一致性：当前统一自动回归通过。
- 玩家核心闭环：关闭开发者模式的 1280×720 可见控件旅程通过；实际窗口已完成学习、进度、暂停/继续、成长、社会入口与保存加载复核。
- 正式 UI：行动、社会、人物与地图顶栏已完成本轮 Windows 11 实际窗口验收。
- Windows Release：当前 x86-64 单 EXE 已从 PowerShell、资源管理器和项目目录外临时位置独立启动通过。
- P0-R1：阶段性健全，Windows 11 Release 冒烟通过；这不是安装程序或跨平台发布结论。
...
2026-07-15 本轮最终验收使用同一精确引擎与 Compatibility 渲染器：统一验证通过综合 `572/572`、逻辑 `35/35`、关闭开发模式的 1280×720 可见控件旅程 `95/95`、安全 `26/26`、状态一致性 `41/41`、模拟质量 `50/50`、Codex 审计 `30/30`，Headless 主项目无解析或脚本加载错误。实际 1280×720 窗口完成新游戏、行动、人物、社会、Space/Esc、保存退出加载；Windows x86-64 Release 又分别从 PowerShell、资源管理器及项目目录外临时位置启动通过。因此当前状态为：P0-R1 阶段性健全，Windows 11 Release 冒烟通过。
