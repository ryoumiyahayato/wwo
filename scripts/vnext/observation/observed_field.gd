...
var _freshness_state: String = CONTRACT.FRESHNESS_UNKNOWN
...
func configure(
	...
	freshness_state_value: Variant = CONTRACT.FRESHNESS_UNKNOWN,
	extensions_value: Variant = {}
) -> bool:
	if typeof(field_id_value) != TYPE_STRING:
		return false
	var candidate_field_id: String = field_id_value as String
	...
	if not CONTRACT.is_valid_confidence(candidate_confidence):
		return false
	...
	if typeof(observed_state_at_value) != TYPE_INT or int(observed_state_at_value) < 0:
		return false
	if typeof(freshness_state_value) != TYPE_STRING:
		return false
	var candidate_freshness: String = freshness_state_value as String
	if not CONTRACT.is_valid_freshness(candidate_freshness):
		return false
	...
	for raw_reference: Variant in provenance_references_value as Array:
		if typeof(raw_reference) != TYPE_STRING:
			return false
		var reference: String = raw_reference as String
		if (
			not CONTRACT.is_valid_provenance_reference(reference)
			or candidate_provenance.has(reference)
		):
			return false
		candidate_provenance.append(reference)
	candidate_provenance.sort()
	if typeof(extensions_value) != TYPE_DICTIONARY:
		return false
	if not CONTRACT.is_detachable_value(extensions_value):
		return false
	...
	_freshness_state = candidate_freshness
...
## Mutation is intentionally limited to the detached DTO returned to a consumer.
...
func copy_detached() -> VNextObservedField:
	var copied := VNextObservedField.new()
