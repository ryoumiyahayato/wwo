...
## no authoritative owner or runtime snapshot is exposed.
...
func query(request: VNextObservationRequest) -> VNextObservationResponse:
	...
	# Capture exactly one revision and detached truth copy before the hook.
...
func _can_return_field(
	...
	# A sensitive field must be explicitly requested; resolver authority alone
	# cannot turn an omitted capability into implicit truth access.
	if (
		not required_capability.is_empty()
		and not request.requests_capability(required_capability)
	):
		return false
