...
static func success(
	...
	if world_revision_value < 0 or observed_at_value < 0:
		return failure(
			CONTRACT.STATUS_QUERY_UNAVAILABLE,
			observer_actor_id_value,
			world_revision_value,
			maxi(0, observed_at_value),
			"response_time_invalid"
		)
	var response := VNextObservationResponse.new()
	var copied_records: Array[VNextObservationRecord] = []
	for record: VNextObservationRecord in records_value:
		if record == null or record.world_revision() != world_revision_value:
			return failure(
				CONTRACT.STATUS_QUERY_UNAVAILABLE,
				observer_actor_id_value,
				world_revision_value,
				observed_at_value,
				"mixed_world_revision"
			)
		var copied_record: VNextObservationRecord = record.copy_detached()
	...
	response._records = copied_records
...
static func failure(
	...
	response._observer_actor_id = observer_actor_id_value
	response._world_revision = world_revision_value
	response._observed_at = maxi(0, observed_at_value)
...
func fingerprint() -> String:
	return CONTRACT.fingerprint_for(_semantic_dictionary())
