...
## Shared, transport-safe vocabulary for observations. This class has no
## authorization or world-state ownership; it only defines the boundary data.
...
static func is_valid_token(candidate_value: String) -> bool:
	...
	for character_index: int in candidate_value.length():
		var candidate_character: String = candidate_value.substr(character_index, 1)
...
static func is_valid_freshness(candidate_value: String) -> bool:
	return (
		candidate_value == FRESHNESS_FRESH
		or candidate_value == FRESHNESS_STALE
		or candidate_value == FRESHNESS_UNKNOWN
	)


static func is_valid_confidence(candidate_value: float) -> bool:
	return candidate_value >= 0.0 and candidate_value <= 1.0


static func is_valid_provenance_reference(candidate_value: String) -> bool:
	return not candidate_value.is_empty() and candidate_value == candidate_value.strip_edges()
