...
func configure(
	...
	var candidate_subject_ids: Array[String] = _read_stable_id_list(subject_ids_value, "subject_ids")
	if not _validation_error.is_empty():
		return false
	var candidate_requested_fields: Array[String] = _read_token_list(
		requested_fields_value, "requested_fields", false
	)
	if not _validation_error.is_empty():
		return false
	var candidate_requested_capabilities: Array[String] = _read_token_list(
		requested_capabilities_value, "requested_capabilities", true
	)
	if not _validation_error.is_empty():
		return false
...
func is_valid() -> bool:
	return (
		...
		and _subject_ids.size() > 0
		and CONTRACT.is_sorted_unique(_subject_ids)
		and _requested_fields.size() > 0
...
func requests_field(field_id: String) -> bool:
	return _requested_fields.has(field_id)
...
func _read_stable_id_list(value: Variant, field_name: String) -> Array[String]:
	...
	for item: Variant in (value as Array):
		if typeof(item) != TYPE_STRING:
			_reject(field_name + "_contains_non_string")
			return []
		var candidate: String = item as String
		if not STABLE_ID.is_valid(candidate):
...
func _read_token_list(value: Variant, field_name: String, allow_empty: bool) -> Array[String]:
	var result: Array[String] = []
	if typeof(value) != TYPE_ARRAY:
		_reject(field_name + "_required")
		return result
