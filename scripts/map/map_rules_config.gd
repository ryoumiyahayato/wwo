...
func load_from_file(path: String = DEFAULT_PATH) -> Error:
	...
	var fields: PackedStringArray = [
		"tile_width", "tile_height", "min_zoom", "max_zoom", "zoom_step",
		"pan_visible_margin", "weak_control_threshold", "contested_threshold",
		"capture_strength_threshold", "capture_contested_threshold",
		"consolidation_strength", "pressure_strength_loss",
		"pressure_contested_gain", "pressure_enemy_gain"
	]
	for field: String in fields:
		if not data.has(field) or (not data[field] is int and not data[field] is float):
	...
	if min_zoom <= 0.0 or max_zoom < min_zoom or zoom_step <= 0.0:
		...
		return ERR_INVALID_DATA
	for value: float in [
		weak_control_threshold, contested_threshold, capture_strength_threshold,
		capture_contested_threshold, consolidation_strength,
		pressure_strength_loss, pressure_contested_gain, pressure_enemy_gain
	]:
		if value < 0.0 or value > 1.0:
			error_message = "地图控制阈值必须位于 0 至 1"
			return ERR_INVALID_DATA
	return OK
