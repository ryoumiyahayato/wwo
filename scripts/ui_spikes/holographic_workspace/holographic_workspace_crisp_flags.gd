		...
		for polygon_value: Variant in (_flag_screen_polygons.get(entity_id, []) as Array):
			...
			for index: int in range(merged.size()):
				if not _screen_polygon_bounds(merged[index]).grow(1.2).intersects(polygon_bounds):
					continue
				var union_result: Array[PackedVector2Array] = Geometry2D.merge_polygons(merged[index], polygon)
				if union_result.size() == 1:
					merged[index] = union_result[0]
					absorbed = true
					break
			if not absorbed:
...
func _admin1_unit_lines_for_iso(iso: String) -> Array[Dictionary]:
	if _world_admin1_unit_cache.has(iso):
		return _world_admin1_unit_cache.get(iso) as Array[Dictionary]
...
func _draw_selected_admin1_on_globe() -> void:
	...
	for entry: Dictionary in _admin1_unit_lines_for_iso(iso):
		var record: Dictionary = entry.get("record", {}) as Dictionary
		for unit_line: PackedVector3Array in (entry.get("unit_lines", []) as Array):
