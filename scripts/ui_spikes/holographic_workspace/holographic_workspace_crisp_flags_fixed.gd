...
func _draw_global_world() -> void:
	...
	# The interactive cache is intentionally populated before the full static
	# triangle cache.  Treat either validated buffer as a usable political
	# surface so startup never exposes the simplified physical triangulation
	# while the static build is still warming.
	var partial_political_ready := (
	...
	if not _presentation_surface_ready():
		...
		# wedges while the closure-validated political cache is still building.
		# Keep the player-facing warmup on the same political triangles that will
		# later receive the full flag layer.
		if map_render_phase == MAP_PHASE_LAND_ONLY or not partial_political_ready:
			if not map_debug_hide_physical_land:
				_draw_physical_land_base()
		elif map_render_phase != MAP_PHASE_LAND_ONLY:
...
func _draw_solid_political_fills() -> void:
	...
	var phase_alpha: float = (
		...
		else (1.0 if map_render_phase == MAP_PHASE_HISTORICAL_FLAGS else 0.72)
	)
		...
		if compact_interactive:
			...
			for point_index: int in range(0, compact_points.size(), 3):
				...
				if not is_finite(triangle_area) or triangle_area <= MAP_SCREEN_TRIANGLE_AREA_EPSILON:
					...
					continue
				var polygon := _expand_screen_triangle_for_fill(PackedVector2Array([
			...
			for record_value: Variant in records:
				...
				var polygon := _expand_screen_triangle_for_fill(record.get("screen", PackedVector2Array()) as PackedVector2Array)
...
func _draw_country_flag_triangles(
	...
	if triangle_count <= 0:
		...
		if bool(empty_stage.get("front_facing_geometry", false)):
			_record_runtime_geometry_failure(
				...
				"front-facing entity had no screen triangle records"
			)
		return
	...
	var all_rejections_are_neutral_surface_fallbacks := true
	points.resize(0)
	uvs_buffer.resize(0)
	colors.resize(0)
	if compact_interactive:
	...
	for record_index: int in range(records.size()):
		...
		if source_polygon.size() != 3 or not _is_valid_screen_polygon(source_polygon):
			all_rejections_are_neutral_surface_fallbacks = false
			_record_map_render_rejection(entity_id, "invalid_flag_triangle")
			_record_runtime_geometry_failure(
				entity_id,
				str(record.get("source_component", entity_id)),
				int(record.get("source_triangle", -1)),
				"DRAW_SUBMISSION_FAILED",
				"flag triangle failed screen or UV shape validation"
			)
			continue
		if uvs.size() != 3:
			var source_triangle_id := int(record.get("source_triangle", -1))
			var representation := str(record.get("representation", ""))
			# Interaction-LOD marker triangles intentionally have no texture
			# coordinates and source_triangle=-1.  The same marker is also used
			# for an intentionally sub-pixel minimum surface. The authoritative
			# neutral political fill remains underneath it; neither presentation
			# fallback is a failed source-triangle submission.
			var neutral_surface_fallback := (
				source_triangle_id < 0
				and (
					_last_map_cache_lod == "interactive"
					or representation == "MINIMUM_DRAWABLE_SURFACE"
				)
			)
			if neutral_surface_fallback:
				_record_map_render_fallback(
					entity_id,
					"INTERACTION_LOD_NEUTRAL_SURFACE" if _last_map_cache_lod == "interactive" else "MINIMUM_DRAWABLE_SURFACE"
				)
				continue
			all_rejections_are_neutral_surface_fallbacks = false
			_record_map_render_rejection(entity_id, "missing_flag_uvs")
			_record_runtime_geometry_failure(
				entity_id,
				str(record.get("source_component", entity_id)),
				int(record.get("source_triangle", -1)),
				"DRAW_SUBMISSION_FAILED",
				"full-resolution flag triangle has no source-owned UVs"
			)
			continue
		# Keep the exact projected source/clipped triangle here.  Its UVs remain
		# owned by the source vertices (or clipped-edge interpolation) and the
		# renderer must not expand the textured triangle independently, which can
		# double-blend adjacent flag fragments into a visible mesh.
		var polygon := source_polygon
		...
		if not bool(draw_validation.get("valid", false)):
			...
			_record_runtime_geometry_failure(
				...
				int(record.get("source_triangle", -1)),
				"DRAW_SUBMISSION_FAILED",
				str(draw_validation.get("reason", "renderer validation failed"))
			)
	...
	if drawn_count > 0:
		if MAP_USE_EXPLICIT_TRIANGLE_SUBMISSION:
			# The buffers duplicate all three vertices for every source/clipped
			# triangle, so batching a country does not merge UV ownership across
			# triangle edges. Submit the already-triangulated stream once instead of
			# asking CanvasItem to triangulate thousands of three-point polygons.
			RenderingServer.canvas_item_add_triangle_array(
				get_canvas_item(),
				_sequential_triangle_indices(points.size()),
				points,
				colors,
				uvs_buffer,
				PackedInt32Array(),
	...
	if drawn_count == 0 and triangle_count > 0 and not all_rejections_are_neutral_surface_fallbacks:
		_record_runtime_geometry_failure(
			...
			-1,
			"DRAW_SUBMISSION_FAILED",
			"all visible flag records were rejected before draw submission"
		)
...
func _draw_historical_entity_borders() -> void:
	...
	var ordinary_pairs := PackedVector2Array()
	var contested_pairs := PackedVector2Array()
	for entity_key_value: Variant in _historical_outline_polygons.keys():
		var entity_id: String = str(entity_key_value)
		var entity: Dictionary = _country_by_id.get(entity_id, {}) as Dictionary
		var status: String = str(entity.get("status", "sovereign"))
		var provisional: bool = bool(entity.get("provisional", false))
		# Ordinary borders are stable product geometry.  Pulse is reserved for
		# explicit hover/selection feedback and is never mixed into the normal
		# political boundary alpha.
		var color: Color = Color(0.18, 0.28, 0.29, lerpf(0.30, 0.52, close_mix))
		...
		for polygon_value: Variant in (_historical_outline_polygons.get(entity_id, []) as Array):
			...
			elif entity_id == hover_country_id or entity_id == selected_country_id:
				draw_polyline(polygon, color, width, false)
			else:
				for point_index: int in range(polygon.size() - 1):
					if status == "contested" or status == "fragmented":
						contested_pairs.append(polygon[point_index])
						contested_pairs.append(polygon[point_index + 1])
					else:
						ordinary_pairs.append(polygon[point_index])
						ordinary_pairs.append(polygon[point_index + 1])
	if not ordinary_pairs.is_empty():
		draw_multiline(
			ordinary_pairs,
			Color(0.18, 0.28, 0.29, lerpf(0.30, 0.52, close_mix)),
			lerpf(0.58, 1.14, close_mix),
			false
		)
	if not contested_pairs.is_empty():
		draw_multiline(
			contested_pairs,
			Color(0.39, 0.28, 0.22, lerpf(0.30, 0.52, close_mix) * 0.92),
			lerpf(0.62, 1.20, close_mix),
			false
		)
