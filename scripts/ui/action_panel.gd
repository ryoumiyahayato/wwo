...
func _populate_actions() -> void:
	...
	for definition_id: String in _definition_ids:
		...
		action_option.set_item_metadata(
			action_option.item_count - 1, definition_id
		)
	_on_action_selected(action_option.selected)
...
func _on_action_selected(_index: int) -> void:
	...
	permission_check.text = (
		((
			"已具备职位权限：%s"
			if permissions.has(definition.position_permission_required)
			else "缺少职位权限：%s"
	...
	permission_check.button_pressed = (
		definition.position_permission_required.is_empty()
...
func _populate_targets(definition: ActionDefinitionData) -> void:
	...
	match definition.category:
		"build_relationship", "investigate_character":
			...
			if society != null:
				for character_id: String in society.roster.get_background_ids():
					var background: BackgroundCharacterData = society.roster.get_background(
						character_id
					)
					_add_eligible_target(
						definition, background.name, character_id, previous_id
					)
				for character_id: String in society.roster.get_active_ids(false):
					_add_eligible_target(
						definition,
						society.roster.get_active(character_id).name,
						character_id,
						previous_id
					)
		"join_organization":
			...
			if society != null:
				for organization_id: String in society.organizations.get_organization_ids():
					...
					_add_eligible_target(
						definition, organization.name, organization_id, previous_id
					)
		"seek_position":
			target_label.text = "目标组织"
		...
		"promote_policy", "support_control":
			...
			_add_eligible_target(
				definition, map_target_id, map_target_id, previous_id
			)
		_:
...
func _on_begin_pressed() -> void:
	...
	message_label.text = "行动已开始，基础费用和额外投入已扣除。"
	_refresh()
...
func _refresh() -> void:
	...
	var has_permission: bool = (
		...
		or selected_definition.position_permission_required.is_empty()
		or _get_player_permissions().has(
			selected_definition.position_permission_required
		)
	)
	var can_afford: bool = (
		selected_definition != null
		and context_service != null
		and has_player
		and context_service.can_afford(
			selected_definition,
			GameSessionService.player_character,
			extra_funding
		)
	...
	var estimate: String = (
		...
		else "第 %d 小时" % action.estimated_completion_hour
	)
...
func _update_investment_limit(
	...
	if context_service == null or definition == null or not has_player:
		...
		return
	var wealth: int = int(
		GameSessionService.player_character.current_status.get("wealth", 0)
	)
	var available_extra: int = maxi(
		wealth - context_service.get_base_funding_cost(definition), 0
	)
	investment_spin.max_value = mini(
		context_service.get_max_extra_funding(), available_extra
	)
	if investment_spin.value > investment_spin.max_value:
		investment_spin.value = investment_spin.max_value
...
func _get_selected_definition() -> ActionDefinitionData:
	...
	var definition_id: String = str(
		action_option.get_item_metadata(action_option.selected)
	)
