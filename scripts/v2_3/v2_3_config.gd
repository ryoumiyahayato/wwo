...
extends RefCounted
## Validated aggregate for the V2.3 Lille spatial-cognition scenario.
...
func location_records() -> Array:
	return get_document("locations").get("locations", []) as Array


func edge_records() -> Array:
	return get_document("graph").get("edges", []) as Array
...
func _validate() -> void:
	...
	var allowed_types: PackedStringArray = [
		"residence", "workplace", "market", "organization_hall",
		"government_office", "post_office", "public_square",
		"railway_station", "city_centre", "regional_centre",
	]
	...
	if locations.size() < 12:
		errors.append("V2.3 正式地点少于 12 个")
	...
	for raw_mode: Variant in transport_records():
		...
		if mode_id.is_empty() or modes.has(mode_id):
			...
			modes[mode_id] = true
	for required_mode: String in ["walk", "urban_transit", "regional_train"]:
...
func _validate_people(locations: Dictionary) -> void:
	...
	for raw_person: Variant in social_people():
		...
		for field: String in ["home_location_id", "workplace_location_id", "postal_address_location_id"]:
			if not locations.has(str(person.get(field, ""))):
				errors.append("V2.3 人物引用未知地点：%s/%s" % [person_id, field])
	for required_id: String in [
		"character_pierre_lefevre", "character_albert_dumont", "jeanne",
		"character_jules_martin", "character_lucien_moreau",
	]:
