...
static func from_dict(data: Dictionary) -> AiStateData:
	...
	model.current_action_record = DataRecordUtils.to_dictionary(
		data.get("current_action_record", {})
	)
	model.last_action_result = str(data.get("last_action_result", ""))
	...
	if raw_candidates is Array:
		for raw_candidate: Variant in raw_candidates as Array:
			if raw_candidate is Dictionary:
				model.candidate_actions.append(
					(raw_candidate as Dictionary).duplicate(true)
				)
	...
	model.long_term_evaluation_count = int(
		data.get("long_term_evaluation_count", 0)
	)
	return model
...
func to_dict() -> Dictionary:
	return {
		...
		"current_action_record": current_action_record.duplicate(true),
