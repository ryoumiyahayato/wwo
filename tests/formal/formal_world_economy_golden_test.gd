...
func _check_tariff_revenue_restore_typing() -> void:
	var source: Variant = SIMULATION_SCRIPT.new()
	_check(source.initialize(), "tariff-revenue restore source initializes")
	if not source.initialized:
		return
	var base_state: Dictionary = source.get_persistent_state()
	var integral_candidate := base_state.duplicate(true)
	_set_tariff_revenue_value(integral_candidate, 0.0)
	var integral_target: Variant = SIMULATION_SCRIPT.new()
	_check(integral_target.initialize(), "integral-float tariff-revenue target initializes")
	if integral_target.initialized:
		_check(integral_target.restore_persistent_state(integral_candidate), "tariff revenue restore accepts integral float")
		var restored_value: Variant = _tariff_revenue_value(integral_target.get_persistent_state())
		_check(restored_value == 0 and typeof(restored_value) == TYPE_INT, "integral tariff revenue restores as int")

	var fractional_target: Variant = SIMULATION_SCRIPT.new()
	_check(fractional_target.initialize(), "fractional tariff-revenue target initializes")
	if not fractional_target.initialized:
		return
	var before_reject: Dictionary = fractional_target.get_persistent_state().duplicate(true)
	var fractional_candidate := base_state.duplicate(true)
	_set_tariff_revenue_value(fractional_candidate, 0.5)
	_check(not fractional_target.restore_persistent_state(fractional_candidate), "tariff revenue restore rejects fractional float")
...
func _tariff_revenue_value(snapshot: Dictionary) -> Variant:
	return _restore_market(snapshot).get("tariff_revenue_centimes", null)
