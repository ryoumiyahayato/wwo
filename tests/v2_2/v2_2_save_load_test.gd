...
func _run() -> void:
	...
	var first_state: Dictionary = simulation.deterministic_digest()
	...
	var second_state: Dictionary = simulation.deterministic_digest()
	...
	test.equal(simulation.deterministic_digest(), second_state, "主存档恢复当前活动、日程、账本和幂等键")
	...
	test.equal(simulation.deterministic_digest(), first_state, "安全备份恢复到上一次有效状态")
	...
	test.equal(simulation.deterministic_digest(), first_state, "版本拒绝不破坏当前运行状态")
	...
	test.equal(simulation.deterministic_digest(), first_state, "损坏存档拒绝后状态不变")
	_cleanup()
	test.finish(self, "V2.2 save/load")
