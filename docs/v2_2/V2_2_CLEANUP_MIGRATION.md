...
本轮将已经通过实机评审的世界地图从旧 `prototype_v2` 路径迁入正式 `world_map` 模块，并删除不合格旧原型的可执行入口和失效辅助面板。
...
迁移后的 `scripts/world_map/internal/` 中仍保留少量 `PrototypeV2*` 全局类名。这些名称只作为现有 GDScript 类型连接的内部兼容标识，不再对应旧场景、旧菜单、旧网格地图或旧数据路径。正式运行入口使用 `WorldMapMain` 与 `WorldMapCanvas`，并通过显式脚本路径连接内部实现，避免依赖陈旧的全局类缓存路径。
